...
-- insert into users (name) values ('tamfu'), ('steve'), ('melcore'), ('mc-tominay');
...
inner join (select t.id, u.id as user_id, u.name as username, s.name as skill_name from skills_tree t inner join job_skills s on t.job_skill_id = s.id inner join users u on t.graduate_id = u.id) t on c.graduate_id = t.user_id;
